...
public class SecurityConfiguration {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
            .cors(Customizer.withDefaults()) // 2. Enable CORS in Spring Security
            .csrf(csrf -> csrf.disable()) // Disable CSRF since we are using JWT
            .authorizeHttpRequests(auth -> auth
                // These are the "Public" doors
                .requestMatchers(
                    "/api/auth/**", 
                    "/v3/api-docs/**", 
                    "/swagger-ui/**", 
    ...
    // 3. Define the CORS configuration source bean so Spring Security permits localhost:5173
    @Bean
